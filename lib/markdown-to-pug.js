...
;(function()
{
      ...
      switch (e.type)
      {
        ...
        case 'heading_open':
          ...
          else
          {
            ...
            ret += this.indent('div'+this.indent(this.tag(e.tag, e.content, e.attrs), (e.level+level)), e.level+level - 1);
          }
          break;
          ...
          if (e.children)
          {
            ...
            e.children.forEach((c) =>
            {
              ...
              switch (c.type)
              {
                ...
                case 's_close':
                case 'link_close':
                  ret += this.indent("  |", (e.level+level - 1))
                default:
                  break;
              }
            })
  ...
  p.injectedTag = function(tag, src, attrs, inline)
  {
    let attributes = (attrs != null && attrs != undefined) && '(' + attrs.map(e => e[0]+'="'+e[1]+'"') + ')' || '';
    return 'div\n  ' + tag + attributes + (inline ? src : this.newline(src));
  }
